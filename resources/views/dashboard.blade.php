...
<head>
        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    colors: { brand: { 50: '#f0fdf4', 100: '#dcfce7', 500: '#22c55e', 700: '#15803d', 900: '#2b5329' } }
                }
            }
        }
    </script>
    ...
    <style>
        ...
        /* Tab Sawah */
        .sawah-tab { transition: all 0.2s; cursor: pointer; }
        .sawah-tab.active {
            background: #2b5329;
            color: white;
            box-shadow: 0 4px 12px -2px rgba(43, 83, 41, 0.35);
        }
        .sawah-tab:not(.active):hover { background: #f0fdf4; color: #2b5329; }
        .sawah-tab .dot { width: 8px; height: 8px; border-radius: 50%; }

        /* Pagination */
        .pagination-btn { display: inline-flex; align-items: center; justify-content: center; min-width: 32px; height: 32px; padding: 0 8px; border-radius: 8px; border: 1px solid #e5e7eb; background: #fff; color: #6b7280; font-size: 13px; cursor: pointer; transition: all 0.15s; }
        .pagination-btn:hover:not(:disabled) { background: #f3f4f6; color: #111827; }
        .pagination-btn.active { background: #f3f4f6; border-color: #d1d5db; color: #111827; font-weight: 600; }
        .pagination-btn:disabled { opacity: 0.4; cursor: not-allowed; }

        /* Dummy overlay for locked tabs */
        .locked-overlay { pointer-events: none; opacity: 0.45; filter: grayscale(0.3); }

        /* Filter tab */
        .filter-tab { transition: all 0.15s; cursor: pointer; }
        .filter-tab.active { background: #2b5329; color: white; border-color: #2b5329; }
        .filter-tab:not(.active):hover { background: #f0fdf4; border-color: #22c55e; color: #15803d; }

        /* Print styles */
       @media print {
    /* 1. Sembunyikan elemen dashboard yang tidak perlu dicetak */
    nav, .sidebar, button, .btn, .refresh-btn, .pilih-lahan, .card-status, .alert, select {
    ...
    table {
        ...
        table-layout: auto !important; /* Biar kolom menyesuaikan isi secara alami */
    }
    ...
    td:first-child, th:first-child {
        width: 25% !important;
        white-space: normal !important; /* Izinkan patah baris jika format tanggalnya panjang */
    }
}
    </style>
</head>
...
<body class="min-h-screen text-gray-800 antialiased pb-12">
    ...
    <main class="max-w-7xl mx-auto px-6 md:px-10 py-8">
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between gap-4">
            ...
            <div id="alert-container" class="bg-white border px-4 py-2 rounded-xl flex items-center gap-3 shadow-sm self-start">
        ...
        <div id="siklus-tanam-container" class="mb-6 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                ...
                <div class="flex items-center gap-2 no-print">
                    <button onclick="window.mulaiTanamBaru()" class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all flex items-center gap-1">
                        <i class="fas fa-seedling"></i> Mulai Tanam Baru
                    </button>
                    <button onclick="window.resetSiklusTanam()" class="px-3 py-1.5 bg-gray-100 hover:bg-red-50 text-gray-600 hover:text-red-600 text-xs font-bold rounded-xl border transition-all flex items-center gap-1">
                        <i class="fas fa-undo"></i> Reset 0
                    </button>
                </div>

                <span id="siklus-badge-status" class="px-3 py-1 rounded-full text-xs font-bold bg-gray-50 text-gray-500 border">
                    Memuat status...
                </span>
            </div>
            ...
            <div class="relative w-full h-3 bg-gray-100 rounded-full overflow-hidden mb-2">
                ...
                <div id="slider-posisi-hari" class="absolute top-1/2 left-[0%] -translate-y-1/2 w-3 h-3 bg-white border-2 border-brand-900 rounded-full shadow transition-all duration-500"></div>
            </div>
            ...
            <div id="alert-ultrasonik" class="mt-5 p-4 bg-red-50 border border-red-100 rounded-xl flex items-start gap-3 hidden">
        ...
        <div id="dummy-banner" class="hidden mb-6 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3 flex items-center gap-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="stat-card bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span id="servo1-badge" class="px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-400">OTOMATIS</span>
                    </div>
                    <div class="flex items-baseline gap-1">
                    ...
                    <div class="mt-4 flex gap-2">
                        <button id="btn-buka" onclick="window.controlServo(1)" class="flex-1 py-2 bg-gray-100 text-gray-400 text-[10px] font-bold rounded cursor-not-allowed" disabled>BUKA</button>
                        <button id="btn-tutup" onclick="window.controlServo(2)" class="flex-1 py-2 bg-gray-100 text-gray-400 text-[10px] font-bold rounded cursor-not-allowed" disabled>TUTUP</button>
                    </div>
                </div>
                ...
                <div class="stat-card bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span id="servo2-badge" class="px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-400">IDLE</span>
                    </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex flex-wrap items-center gap-3">
                    ...
                    <div class="ml-auto flex flex-wrap items-center gap-2 no-print">
                        ...
                        <button onclick="setFilter('hari')" id="f-hari" class="filter-tab px-3 py-1.5 rounded-lg text-xs font-semibold border border-transparent text-gray-500">Hari Ini</button>
                            <button onclick="setFilter('minggu')" id="f-minggu" class="filter-tab px-3 py-1.5 rounded-lg text-xs font-semibold border border-transparent text-gray-500">Minggu Ini</button>
                            <button onclick="setFilter('bulan')"  id="f-bulan"  class="filter-tab px-3 py-1.5 rounded-lg text-xs font-semibold border border-transparent text-gray-500">Bulan Ini</button>
                            <button onclick="setFilter('tahun')"  id="f-tahun"  class="filter-tab px-3 py-1.5 rounded-lg text-xs font-semibold border border-transparent text-gray-500">Tahun Ini</button>
                            <button onclick="setFilter('semua')"  id="f-semua"  class="filter-tab active px-3 py-1.5 rounded-lg text-xs font-semibold border border-transparent">Semua</button>
                        </div>

                        <!-- Tombol Cetak -->
                        <button onclick="cetakLaporan()" class="flex items-center gap-2 px-4 py-2 bg-brand-900 hover:bg-brand-700 text-white text-xs font-bold rounded-xl transition-all">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm" style="table-layout: fixed;">
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 border-b border-gray-100"><i class="fas fa-clock mr-1"></i>Waktu</th>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 border-b border-gray-100">Air (cm)</th>
                                </tr>
                            </thead>
                            <tbody id="log-table-body" class="divide-y divide-gray-50">
                                <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400 italic text-sm">Menghubungkan ke Firebase...</td></tr>
                            </tbody>
                        </table>
                    </div>
    ...
    function switchSawah(n) {
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            if (tab) {
                if (i === n) {
                    tab.className = 'sawah-tab active flex items-center gap-3 px-5 py-3 rounded-2xl border border-transparent';
                } else {
                    tab.className = 'sawah-tab flex items-center gap-3 px-5 py-3 rounded-2xl border border-gray-200 bg-white text-gray-600';
                }
            }
        }
        ...
        if (n === 1) {
            ...
            if (elPirStatus) {
                ...
                elPirStatus.className = d.pir === 'Bahaya' ? 'text-2xl font-bold text-red-600' : 'text-2xl font-bold text-emerald-600';
            }
            if (elPirDot) {
                elPirDot.className = d.pir === 'Bahaya' ? 'h-4 w-4 rounded-full bg-red-600 animate-pulse' : 'h-4 w-4 rounded-full bg-emerald-500';
            }
        }
    }
</script>
...
<script type="module">
    ...
    import { getDatabase, ref, onValue, set, query, limitToLast } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js";
    ...
    // Penyimpanan data lokal untuk sistem Firebase ter-cache
    ...
    const waterChart = new Chart(waterCtx, {
        ...
        options: {
            ...
            plugins: { legend: { display: false } },
            scales: {
                y: { beginAtZero: true, ticks: { callback: v => v + ' cm' }, grid: { color: 'rgba(0,0,0,0.04)' } },
                x: { grid: { display: false } }
            }
        }
    });
    ...
    onValue(ref(db, 'iot/siklus/tanggal_tanam'), (snapshot) => {
        if (activeSawah !== 1) return; // Kunci pembaruan jika tidak berada di tab riil
        ...
        if (!tanggalTanamStr) {
            ...
            if (badgeStatus) {
                ...
                badgeStatus.className = "px-3 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-400 border";
            }
        ...
        if (infoHari) {
            infoHari.innerText = hitungHari < 1 ? "Persiapan Lahan (Hari 0)" : "Siklus Berjalan: Hari ke-" + hitungHari;
        }
        ...
        // Alokasi visualisasi progress bar fase tanaman
        if (hitungHari <= 120) {
            ...
            if (badgeStatus) {
                ...
                badgeStatus.className = "px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200";
            }
            if (alertUltrasonik) alertUltrasonik.classList.add('hidden');
            ...
            if (badgeStatus) {
                ...
                badgeStatus.className = "px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200";
            }
            ...
            if (badgeStatus) {
                ...
                badgeStatus.className = "px-3 py-1 rounded-full text-xs font-bold bg-red-50 text-red-700 border border-red-200";
            }
            if (alertUltrasonik) alertUltrasonik.classList.add('hidden');
        }
    });
    ...
    // Fungsi global untuk memaksa update UI riil dari cache data database terkini
    ...
    function updateRealtimeUI(data) {
        ...
        if (elPirStatus) {
            ...
            elPirStatus.className = isBahaya ? 'text-2xl font-bold text-red-600' : 'text-2xl font-bold text-emerald-600';
        }
        if (elPirDot) {
            elPirDot.className = isBahaya ? 'h-4 w-4 rounded-full bg-red-600 animate-pulse' : 'h-4 w-4 rounded-full bg-emerald-500';
        }
    ...
    onValue(ref(db, 'iot/latest'), (snapshot) => {
        ...
        databaseCache.latest = data; // Simpan ke objek cache lokal

        // Update UI komponen card utama jika posisi tab berada pada sawah riil (Sawah 1)
        if (activeSawah === 1) {
            updateRealtimeUI(data);
        }

        // Grafik diperbarui secara real-time di latar belakang terlepas dari status navigasi tab
        const now = new Date();
        ...
        chartLabels.push(timeStr); soilData.push(data.soil || 0);
        waterLabels.push(timeStr); waterData.push(data.water || 0);
    ...
    onValue(ref(db, 'iot/control/pintu_air'), (snapshot) => {
        ...
        if (val === 0) {
            ...
            [btnBuka, btnTutup].forEach(b => {
                if (b) {
                    b.disabled = true;
                    b.className = 'flex-1 py-2 bg-gray-100 text-gray-400 text-[10px] font-bold rounded cursor-not-allowed';
                }
            });
        } else {
            ...
            if (btnBuka) {
                ...
                btnBuka.className = val === 1 ? 'flex-1 py-2 bg-blue-600 text-white text-[10px] font-bold rounded shadow-md' : 'flex-1 py-2 bg-blue-400 text-white text-[10px] font-bold rounded hover:bg-blue-500';
            }
            if (btnTutup) {
                btnTutup.disabled = false;
                btnTutup.className = val === 2 ? 'flex-1 py-2 bg-gray-800 text-white text-[10px] font-bold rounded shadow-md' : 'flex-1 py-2 bg-gray-500 text-white text-[10px] font-bold rounded hover:bg-gray-600';
            }
        }
    });
    ...
    onValue(query(ref(db, 'iot/logs'), limitToLast(6)), (snapshot) => {
        ...
        if (data) {
            Object.values(data).reverse().forEach(log => {
                ...
                div.className = `p-3 bg-gray-50 rounded-xl border-l-4 ${log.pir === 'Bahaya' ? 'border-red-500' : 'border-emerald-500'}`;
                div.innerHTML = `
                    <div class="flex justify-between mb-1">
                        <span class="text-[10px] text-gray-400">${log.created_at || 'Baru saja'}</span>
    ...
    window.cetakLaporan = function() {
        ...
        const now = new Date().toLocaleString('id-ID', { weekday:'long', day:'numeric', month:'long', year:'numeric', hour:'2-digit', minute:'2-digit' });
        ...
        if (printSubtitle) printSubtitle.innerText = 'Lahan: Sawah 1 (Blok A) · Total: ' + filteredEntries.length + ' entri';
        if (printGenerated) printGenerated.innerText = 'Dicetak pada: ' + now;
        if (printTotal) printTotal.innerText = 'Total ' + filteredEntries.length + ' entri';

        const saved = { entries: filteredEntries, page: currentPage };
        renderAllForPrint();
        ...
        filteredEntries.forEach((log, i) => {
            ...
            tr.innerHTML = `
                <td style="padding:8px 24px;font-size:12px;color:#6b7280;">
                    <i class="fas fa-calendar-alt" style="margin-right:4px;color:#d1d5db;"></i>${log.created_at || '—'}
                </td>
                <td style="padding:8px 24px;text-align:center;">
                    <span style="padding:2px 10px;border-radius999px;font-size:10px;font-weight:700;background:${isBahaya ? '#fee2e2' : '#d1fae5'};color:${isBahaya ? '#b91c1c' : '#065f46'};">
                        ${isBahaya ? 'Bahaya' : 'Aman'}
                    </span>
                </td>
                <td style="padding:8px 24px;text-align:center;font-weight:600;color:#374151;">${log.soil ?? '—'}</td>
                <td style="padding:8px 24px;text-align:center;font-weight:600;color:#374151;">${log.water != null ? parseFloat(log.water).toFixed(1) : '—'}</td>
            `;
            ...
            pageEntries.forEach((log, i) => {
                ...
                tr.innerHTML = `
                    <td class="px-6 py-3 text-gray-500 text-xs whitespace-nowrap overflow-hidden text-ellipsis">
                        <i class="fas fa-calendar-alt mr-1 text-gray-300"></i>${log.created_at || '—'}
                    </td>
